...
| `ASDLS-spec.md` | The **full normative ASDLS** (author-supplied executive spec): §1 philosophy + priority matrix, §2 palette/hex-typology + 5%-rule + chromatic grammar, §3 materiality/glitch, §4 semiotic symbols, §5 composition + lens prompts, §6 the Tier 0–4 state machine, §7 SPECD formula + keyword library, §8 master checklist. | The **executive art-direction contract** — the source the DALL-E art-direction override (`overrides/album-art-preferences.md`) encodes. Wins over the `ASDLS.md` summary on conflict. |
