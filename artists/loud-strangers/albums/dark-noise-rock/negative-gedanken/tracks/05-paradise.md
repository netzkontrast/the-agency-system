---
title: "Paradise"
track_number: 5
instrumental: false
explicit: false
suno_url: ""
sheet_music:
  pdf: ""
  musicxml: ""
  midi: ""
---

# Paradise

## Track Details

| Attribute | Detail |
|-----------|--------|
| **Track #** | 05 |
| **Title** | Paradise |
| **Album** | [Negative Gedanken](../README.md) |
| **Status** | Not Started |
| **Suno Link** | — |
| **Stems** | No |
| **Instrumental** | No |
| **Explicit** | No |
| **POV** | Das verlorene Ich / Frohnatur (Memoria) — durch Watte, betäubt |
| **Role** | Schwebe — bittersüß. Paradies als Selbst-Betäubung. Dream-Pop-Bridge nimmt T4's „keine Träume mehr / außer in der Nacht"-Outro auf. |
| **Fade Out** | 4s — Crossfade in T6 (Dauerschleife) |
| **Target Duration** | 3:30–5:00 |
| **Sources Verified** | N/A (fiktionaler Quelltext-Frame — verankert in Teil 3 „Möchte fliehen in ein Paradies" + „Betäube mich") |

<!--
SOURCE VERIFICATION: Required for tracks with source material (quotes, real events, etc.)
- ❌ Pending = Sources added, awaiting human verification
- ✅ Verified (DATE) = Human has checked all URLs, quotes, dates, names
- N/A = Track has no external source material

Human must verify BEFORE track moves to production. See CLAUDE.md for verification workflow.
-->





## Concept

T5 ist das einzige englischsprachige Stück des Albums und die einzige „schöne" Klangfläche — ätherisch, harmonisch tröstlich, **und das ist der Trick**. POV: das verlorene Ich, aber **betäubt**. Die Bridge IST die Selbst-Betäubung, nicht reine Sehnsucht. Direkt vor der Paradies-Linie im Quelltext steht „Betäube mich / Mein scheinbar einziger Ausweg" — Dream-Pop-Idiom passt nicht zufällig: die Sängerin spricht durch Watte. Pharmakologie, nicht Erinnerung pur.

Lexicon: **Konjunktiv und Vergangenheit** (per cast.md §4). EN-Übersetzung: „I would see", „I would wear", „I would have been", „if she were me". Die Paradies-Welt wird beschrieben, aber immer im hypothetischen Modus. Niemals Indikativ-Triumph („I am free" wäre disqualifiziert).

Quelltext-Stütze: **Teil 3, sehr stark**:
- „Möchte fliehen in ein Paradies in dem ich mich seh' so wie ich ausseh'"
- „Dass ich mich liebe so wie ich bin"
- „Mich wohl fühle in dieser Kleidung / Dieser Haut / Diesem Angesicht"
- **Anker zum Trug**: „Betäube mich / Mein scheinbar einziger Ausweg" → manifestiert in der Bridge

**Outro-Brücke aus T4** (per README's Lyrical-Motifs-Tabelle): T4 endete mit „Keine Träume mehr / außer in der Nacht" — T5 startet im EN mit dem Echo „no dreams left for me / except in the night / and in the night / I see myself". Direkter narrativer Bezug.

Cross-Track: T7 (Funke) verwirft dieses Paradies als Trugbild („wenn das mein Ich wär'" als Konjunktiv-Echo des hiesigen Konjunktiv-Paradies). T6 zertrümmert die Watte mit mechanischem Loop.

## Cross-References

### References TO This Track

| From Track | Reference Type | Detail |
|------------|---------------|--------|
| — | — | — |

### References FROM This Track

| To Track | Reference Type | Lyric Line | Detail |
|----------|---------------|------------|--------|
| — | — | — | — |

**Reference types:** `callback` (echoes earlier lyric/image), `motif` (recurring thematic element), `character` (same character reappears), `contrast` (deliberate inversion of earlier idea), `resolution` (resolves tension from earlier track)

<!-- END CROSS-REFERENCES -->

## Mood & Imagery

Das T5-Artwork: greyscale wash painting, weich, blurred, foggy — wie durch Kondensation an Glas gesehen. Ein einziger Punkt kalten blau-weißen Lichts oben rechts (das einzige Paradies-Versprechen). Die Figur darunter ist fast komplett im Wash aufgelöst.

Akustisch das gleiche: alles in dichten Reverb und Delay getaucht, die Stimme schwebt, scheint nie zu landen. Aber unter der Schönheit liegt etwas Falsches — die Wärme ist nicht echt, sie ist *pharmazeutisch*. Ein bittersüßes, trügerisches Paradies.

Sensorisch: ätherische multi-layered vocals (vom Hall vervielfacht, nicht durch Multi-tracking), schimmernde Reverb-Tails, langsame Synth-Pads, Slowcore-Drums oder gar keine, washed-out E-Gitarren mit massivem Reverb/Delay. **Die einzige Farbe im Album außer Bone/Black/Bruise** — kühle blau-weiße Hoch-Frequenzen.

## Musical Direction

- **Tempo**: 80–95 BPM (Dream-Pop-Schwebe — nicht zu langsam)
- **Feel**: Ätherisch, schwebend, harmonisch tröstlich AN DER OBERFLÄCHE, mit subtiler Falschheit darunter. Niemals voll triumphal. Keine starken Beats — Atmosphäre dominiert.
- **Instrumentation**: Washed E-Gitarren mit massivem Reverb + Delay (langsam tremoliertes Picking), schimmernde Synth-Pads, sehr weiche Slowcore-Drums (oder gar keine), Sub-Bass nur als sanfter Puls, multi-layered Vocal-Tracks (durch Hall vervielfacht), Shoegaze-Wand bei Pre-Chorus + Chorus
- **Vocal**: Single-Voice, multi-layered durch Reverb (nicht durch echte Multi-tracking — bleibt mono-vocal-Aufnahme, der Hall macht den Chorus-Effekt). Weiche, atemtragende Lieferung. Im Bridge an einem Punkt bricht die Watte kurz — „numb me / my only — " — die Lieferung wird trockener, klarer. Direkt danach: zurück in den Wash.

<!-- SERVICE: suno -->
## Suno Inputs

### Style Box
*Copy this into Suno's "Style of Music" field:*

```
melancholic dream-pop and shoegaze, 85 bpm dream-pop float, ethereal and
deceptively comforting on the surface with subtle wrongness underneath, never
fully triumphant, female single-vocal soft and breathy delivery sounding as
if through cotton or sedation, multi-layered through dense reverb not through
multi-tracking, washed electric guitars with heavy reverb and delay slow
tremolo picking, shimmering synth pads, very soft slowcore drums or no drums
at all, sub-bass gentle pulse only, shoegaze wall in pre-chorus and chorus,
cool blue-white high frequencies, lo-fi cassette warmth, bittersweet
pharmaceutical paradise feeling
```

### Exclude Styles
*Negative prompts — append to Style Box when pasting into Suno (e.g. "no drums, no electric guitar"):*

```
no autotune, no triumphant major-key resolution, no clean studio polish, no
strong beats, no electronic dance beats, no synth lead melody, no rap, no
death metal vocals, no aggressive guitars, no fade-out outro to silence
```

### Lyrics Box
*Copy this into Suno's "Lyrics" field:*

<!-- INSTRUMENTAL TRACKS: If instrumental: true, use only section tags (no sung lyrics).
     Example: [Intro]\n\n[Main Theme]\n\n[Bridge]\n\n[Outro]\n\n[End]
     Set "Instrumental: On" in Suno. -->

<!-- VOCAL TRACKS: WARNING: Suno sings EVERYTHING literally including parenthetical directions.
     NEVER use (whispered), (softly), (screaming), (spoken), (laughing), etc.
     Use metatags like [Whispered] or put delivery notes in the Style Box instead. -->

```
[Intro - washed pad swell, T4 drone fading into shimmer]

[Verse 1]
no dreams left for me
except in the night
and in the night
I see myself

I see myself
the way I look outside
the way I would have looked
if she were me

[Pre-Chorus]
this skin
this face
this hair
mine

[Chorus]
in paradise
I would see this face
in paradise
I would wear this skin

in paradise
I would have been
the one I was

[Verse 2]
how can it be
that the waking world
will not let me in
the way the dream does

how can it be
that I have to drift
just to be allowed
to be

[Pre-Chorus]
this skin
this face
this hair
mine

[Chorus]
in paradise
I would see this face
in paradise
I would wear this skin

in paradise
I would have been
the one I was

[Bridge - the wash thins, vocal slightly drier]
numb me
my only
way out

numb me
my only

[Final Chorus]
in paradise
I would have been
in paradise
I would

[Outro]
in the night
in the night
in the

[End]
```
<!-- /SERVICE: suno -->

<!-- VOCAL TRACKS ONLY: Remove this section for instrumental tracks -->

## Streaming Lyrics

*For distributor submission (Spotify, Apple Music, etc.). No section tags, repeats written out, plain text.*

```
No dreams left for me
Except in the night
And in the night
I see myself

I see myself
The way I look outside
The way I would have looked
If she were me

This skin
This face
This hair
Mine

In paradise
I would see this face
In paradise
I would wear this skin

In paradise
I would have been
The one I was

How can it be
That the waking world
Will not let me in
The way the dream does

How can it be
That I have to drift
Just to be allowed
To be

This skin
This face
This hair
Mine

In paradise
I would see this face
In paradise
I would wear this skin

In paradise
I would have been
The one I was

Numb me
My only
Way out

Numb me
My only

In paradise
I would have been
In paradise
I would

In the night
In the night
In the
```

<!-- END VOCAL ONLY -->

## Production Notes

- **Entry**: T4's Drone-Outro fadet aus, T5's Pad-Swell setzt darüber ein. Crossfade 5-8s — die längste Überblendung des Albums (lässt die Watte langsam ein).
- **Reverb-Wash über alles**: Vocal, Gitarre, Synth — alles in dichtem Hall. Aber Vocal sollte trotzdem verständlich bleiben — Style-Setting: lange Reverb-Tails, mittlere Wet/Dry. Das ist die einzige Track-Stelle des Albums mit Reverb-Heroismus — und sie ist FALSCH, weil sie die Betäubung simuliert.
- **Multi-layer Vocal durch Hall, nicht Multi-tracking**: bewusst MONO-VOCAL-Aufnahme, der dichte Reverb erzeugt den Layered-Eindruck. Mehrfach-Tracking ist T2's Privileg (Stimmen-Chor). T5 ist verlorenes Ich = einsame Stimme im Hall.
- **Bridge „numb me"**: hier soll der Hall kurz reduziert werden — die Stimme wird trockener, klarer, ungeschützter. **Das ist der Moment, in dem die Maske rutscht.** Direkt nach „my only —" zurück in den vollen Wash. Mix-Engineer-Aufgabe.
- **Outro „in the night / in the night / in the —"**: wash fadet langsam. Unter dem Wash fängt bereits **T6's mechanischer Puls** an, leise zu pulsieren — als ob die Realität durch die Betäubung wieder durchsickert. Crossfade direkt in T6.
- **EN-Aussprache**: Die Sängerin ist DE-Sprecherin auf einem DE-Album. EN-Lyrics sollen einen leichten DE-Akzent halten — kein hyperkorrektes Schul-EN. Style-Box: „with subtle German accent, soft EN delivery" — verstärkt die „Watte"-Wirkung.
- **„How can it be" (Quelltext-Echo)**: Quelltext „Doch wie kann es sein dass das im wahren Leben nicht auch geht?" — wörtlich übersetzt als rhetorische Frage. Sitzt direkt vor dem zweiten Chorus, hält das Konjunktiv-Frame aufrecht.
<!-- SERVICE: suno -->
- **V5/V5.5**: Dream-Pop ist Suno's bestes Genre. Eine Generation reicht meist. Bei zu glücklicher/triumphaler Generation: Style Box „melancholic, sedated, never triumphant" verstärken. Wenn V5 zu viel Drums setzt: „no drums or very soft slowcore drums" wiederholen.
- **EN mit DE-Akzent**: V5 kann das simulieren mit „female vocal subtle German accent, soft and breathy"; wenn die Generation zu American Indie klingt, das nachschärfen.
<!-- /SERVICE: suno -->

<!-- VOCAL TRACKS ONLY: Remove these sections for instrumental tracks -->

## Pronunciation Notes

**This table is a mandatory checklist, not passive documentation.** Every entry below MUST be applied as phonetic spelling in the Suno Lyrics Box. Before finalizing: read each row, search the Suno lyrics for the standard spelling, and confirm the phonetic version is used.

| Word/Phrase | Pronunciation | Reason |
|-------------|---------------|--------|
| — | — | T5 ist komplett EN — keine DE-Pronunciation-Risks. Aber: leichter DE-Akzent in der EN-Vocal-Delivery erwünscht (siehe Production Notes — Style Box: „female vocal subtle German accent, soft and breathy"). |

<!-- SERVICE: suno -->
## Phonetic Review Checklist

**Review before generating on Suno:**

- [ ] **Proper nouns scanned**: All names, places, brands identified
- [ ] **Foreign names**: Spanish/non-English names use phonetic spelling
- [ ] **Homographs checked**: No ambiguous words (live, lead, read, wind, tear)
- [ ] **Acronyms**: Spelled out (F-B-I, G-P-S, not FBI, GPS)
- [ ] **Numbers**: Year formats checked ('93 not ninety-three)
- [ ] **Tech terms**: Linux → Lin-ucks, SQL → sequel, etc

**Proper nouns in this track:**
| Word | Current | Phonetic | Fixed? |
|------|---------|----------|--------|
| — | — | — | — |
<!-- /SERVICE: suno -->

<!-- END VOCAL ONLY -->

## Generation Log

Mark keepers with ✓ in the Rating column. Checkpoint verification looks for at least one ✓ per track.

| # | Date | Model | Result | Notes | Rating |
|---|------|-------|--------|-------|--------|
| — | — | — | — | — | — |

<!-- OPTIONAL: SoundCloud Waveform Art - Delete if not using this feature -->

## Waveform Art

### ChatGPT Prompt
*Use this prompt with ChatGPT/DALL-E to generate waveform background art (2480x800px):*

```
Generate a wide cinematic image at 2480x800 pixels for use as a SoundCloud waveform background.

[SCENE DESCRIPTION]

Style: [style keywords]
Color palette: [colors]
Mood: [mood keywords]

Important: The image will have an audio waveform overlaid on top, so avoid fine details in the center-middle area.
```

### Waveform Art Link
| Generated | Link |
|-----------|------|
| — | — |

<!-- END WAVEFORM ART -->
