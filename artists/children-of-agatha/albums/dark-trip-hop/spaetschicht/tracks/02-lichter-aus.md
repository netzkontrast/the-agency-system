---
title: "Lichter aus"
track_number: 2
status: "Not Started"
instrumental: false
explicit: false
suno_url: ""
sheet_music:
  pdf: ""
  musicxml: ""
  midi: ""
---

# Lichter aus

## Track Details

| Attribute | Detail |
|-----------|--------|
| **Track #** | 02 |
| **Title** | Lichter aus |
| **Album** | [Spätschicht](../README.md) |
| **Status** | Not Started |
| **Suno Link** | — |
| **Stems** | No |
| **Instrumental** | No |
| **Explicit** | No |
| **POV** | Kollektives Wir |
| **Role** | Setup — die Spätschicht schläft ein, die Station kippt uns zu |
| **Fade Out** | 5s |
| **Target Duration** | — |
| **Sources Verified** | N/A |

## Concept

Das Personal löscht das Licht, die Spätschicht zieht sich zurück oder schläft am Tresen. In dem Moment, wo die Aufsicht wegfällt, gehört die Station uns. Der Raum kippt von Verwahrung zu Möglichkeit. Stille, dann das erste gemeinsame Atmen des Wir.

## Mood & Imagery

Lichtschalter klacken, der Flur wird dunkel bis auf Notbeleuchtung. Schritte entfernen sich. Ein kollektives Innehalten, dann ein Lächeln im Dunkeln. Grün-cyan, lange Schatten.

## Musical Direction

- **Tempo**: ~72 BPM
- **Feel**: Verhalten, lauernd; Bass schwillt aus der Stille
- **Instrumentation**: Sub-Bass, sparsame Breaks, Raumklang, Schalter-Foley, Orgel-Drone

<!-- SERVICE: suno -->
## Suno Inputs

### Style Box
*Copy this into Suno's "Style of Music" field:*

```
dark trip hop, cinematic bass, ~72 BPM, hushed and lurking, deep sub-bass heartbeat, sparse fragmented breaks, sacral organ drone, deadpan German spoken verses with longing sung female "we" hook, hospital corridor reverb, distant footsteps
```

### Exclude Styles

```
no four-on-the-floor, no EDM drop, no bright pop production
```

### Lyrics Box
*Copy this into Suno's "Lyrics" field:*

```
[Verse 1]
Spaetschicht, halb eins.
Schluessel klingeln, dann kein Klang.
Letzter Becher auf dem Tresen,
der Neonstreifen summt allein.
Die Tuer faellt ins Schloss —
Schritte weg, Flur dunkel, Notlicht gruen.

[Chorus]
Lichter aus — wir atmen.
Der Flur gehoert uns jetzt.
Lichter aus — kein Schichtwechsel.
Nur die Nacht, die uns versetzt.

[Verse 2]
Jemand schiebt den Medikamenten-Wagen.
Jemand lacht — einmal, kurz.
Linoleum haelt den Fuss,
die Kapelle haelt die Stille.
Klinkerstein seit neunzehnhundertfuenf,
kein offizielles Wort.
Wir stehen auf, wir stehen zusammen.

[Bridge]
Eine Bombe hat die Kapelle verfehlt.
Ein Feuer wurde aufgehalten.
Wir haben kein Geluebde abgelegt —
aber wir nehmen die Nacht.

[Outro]
Lichter aus.
Wir.
```
<!-- /SERVICE: suno -->

## Streaming Lyrics

```
Spätschicht, halb eins.
Schlüssel klingeln, dann kein Klang.
Letzter Becher auf dem Tresen,
der Neonstreifen summt allein.
Die Tür fällt ins Schloss —
Schritte weg, Flur dunkel, Notlicht grün.

Lichter aus — wir atmen.
Der Flur gehört uns jetzt.
Lichter aus — kein Schichtwechsel.
Nur die Nacht, die uns versetzt.

Jemand schiebt den Medikamentenwagen.
Jemand lacht — einmal, kurz.
Linoleum hält den Fuß,
die Kapelle hält die Stille.
Klinkerstein seit neunzehnhundertfünf,
kein offizielles Wort.
Wir stehen auf, wir stehen zusammen.

Eine Bombe hat die Kapelle verfehlt.
Ein Feuer wurde aufgehalten.
Wir haben kein Gelübde abgelegt —
aber wir nehmen die Nacht.

Lichter aus.
Wir.
```

## Production Notes

- Strophen: deadpan/blunt Sprechgesang; Hook: sehnsüchtiges gesungenes „Wir"
- Dynamik aus der Stille aufbauen — der Bass als erstes Lebenszeichen

## Pronunciation Notes

| Word/Phrase | Pronunciation | Reason |
|-------------|---------------|--------|
| Spätschicht | Spaetschicht | ä→ae umlaut conversion for Suno |
| Medikamentenwagen | Medikamenten-Wagen | Hyphen added in Suno box to prevent compound mis-split |
| neunzehnhundertfünf | neunzehnhundertfuenf | ü→ue; long numeral phrase, read as a single entity |
| Gelübde | Geluebde | ü→ue; liturgical term, unusual for Suno vocabulary |
| Klinkerstein | KLINK-er-shtine | Suno may anglicise "kl" onset; stress on first syllable |
| versetzt | fer-ZETST | Standard German, included for completeness on hook word |

## Generation Log

| # | Date | Model | Result | Notes | Rating |
|---|------|-------|--------|-------|--------|
| — | — | — | — | — | — |
