---
title: "Biege dich"
track_number: 1
status: "Not Started"
instrumental: false
explicit: false
suno_url: ""
sheet_music:
  pdf: ""
  musicxml: ""
  midi: ""
---

# Biege dich

## Track Details

| Attribute | Detail |
|-----------|--------|
| **Track #** | 01 |
| **Title** | Biege dich |
| **Album** | [Anguillismus](../README.md) |
| **Status** | Not Started |
| **Suno Link** | — |
| **Stems** | No |
| **Instrumental** | No |
| **Explicit** | No |
| **POV** | Aalprophet:in (Ich/Wir — Gemeinde) |
| **Role** | Opener — etabliert Doktrin, Wasser-Motiv und das vollständige Glaubensbekenntnis |
| **Fade Out** | 5s |
| **Target Duration** | 4:30–5:30 |
| **Sources Verified** | N/A |

## Concept

Erstes Gebot: **Du sollst dich anpassen und fließen.** Wie der Aal sich durch
den engsten Spalt windet, so soll auch der Gläubige sich den Strömungen des
Lebens anpassen — flexibel sein, im Sturm nicht brechen, sondern sich biegen.
*Starrsinn ist die Sünde des trockenen Holzes.* Der Opener öffnet die Liturgie
aus der Stille der Stationsnacht heraus und stellt das volle Bekenntnis auf:
„Glitschig sei mein Pfad, tief mein Gewässer, ungreifbar mein Geist." Hier wird
der Hörer in den Glauben getauft — ruhig, dunkel, einladend.

## Mood & Imagery

Dunkles Wasser, das langsam steigt. Das Summen der Leuchtstoffröhren wie ein
ferner Choral. Ein Körper, der nachgibt statt zu zerbrechen. Trockenes Holz, das
splittert — und daneben der Aal, der sich biegt und weiterzieht. Erste Gezeite.

## Musical Direction

- **Tempo**: sehr langsam, ~60 BPM (gefühlte Drift, kaum Puls)
- **Feel**: tiefer, einladender Drone; erste Tide; aus Stille aufsteigend
- **Instrumentation**: Sub-Bass-Bett, gefluteter Hall, fernes Neon-Summen, tiefe halb gesprochene Stimme, erste geschichtete „Amen"-Antwort am Schluss

<!-- SERVICE: suno -->
## Suno Inputs

### Style Box
```
dark ambient, ritual drone, very slow ~60 BPM, hypnotic, submerged sub-bass like a rising tide, flooded-corridor reverb, distant fluorescent hum, low intimate close-mic German spoken-sung liturgy, layered whispered congregation Amen, solemn, aqueous, clinical, ironic-sublime
```

### Exclude Styles
```
no drums, no bright synths, no upbeat rhythm
```

### Lyrics Box
*Wird von `/bitwize-music:lyric-writer` gefüllt (Track 1 zuerst — etabliert das vollständige Glaubensbekenntnis als Refrain-Anker).*

```
[Lyrics folgen]
```
<!-- /SERVICE: suno -->

## Streaming Lyrics

```
[folgt nach lyric-writer]
```

## Production Notes

- Das Glaubensbekenntnis hier **vollständig** und klar — es ist der Anker, auf den alle weiteren Tracks zurückgreifen.
- Schluss-„Amen" als geflüsterte, mehrfach geschichtete Gemeinde-Antwort (Schluss-Siegel-Motiv des Albums).
