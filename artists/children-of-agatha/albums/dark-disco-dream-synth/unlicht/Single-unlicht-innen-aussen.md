---
title: "Unlicht (Innen/Außen)"
track_number: 0
status: "In Progress"
instrumental: false
explicit: false
suno_url: ""
sheet_music:
  pdf: ""
  musicxml: ""
  midi: ""
---

# Unlicht (Innen/Außen)

## Track Details

| Attribute | Detail |
|-----------|--------|
| **Track #** | Single — Parallel-Version |
| **Title** | Unlicht (Innen/Außen) |
| **Album** | Standalone Single — Children of Agatha × the Agency System (Crossover) |
| **Genre** | Dark Disco Dream Synth / Dystopian Future Synth |
| **Status** | In Progress |
| **Suno Link** | — |
| **Stems** | No |
| **Instrumental** | No |
| **Explicit** | No |
| **POV** | Wounded Healer — Außen-Selbst (ANP) ⇄ Innen-Selbst (EP); vier frei alternierende Stimmprofile, m/w driftend |
| **Role** | **Parallel-Version** zu „Unlicht" — konkreter, klarer. Innen/Außen als strukturelles Rückgrat, der Wounded Healer als benannte Figur, Research-Bilder explizit |
| **Fade Out** | langer Trance-Outro-Fade |
| **Target Duration** | 5:00–7:00 |
| **Sources Verified** | N/A |

> **Parallel-Version (verbindlich).** Dies ist die *konkrete/klare* Schwesterfassung von [`Single-unlicht.md`](Single-unlicht.md) — gleiche Figur, gleicher Bogen, gleiches Genre, aber: (1) **Innen/Außen** wird das hörbare Gerüst (Außen-Strophen = ANP/Heiler-Oberfläche, Innen-Strophen = EP/Wunde); (2) der **Wounded Healer** (R2: „it is his own hurt that gives a measure of his power to heal"; Chiron) wird *benannt*, nicht nur angedeutet; (3) die **Research** (R1–R5 in [`RESEARCH.md`](RESEARCH.md)) tritt als konkretes Bild in den Text. Würde-Linie, name_exposure und „noch"-Tether bleiben hart.

## Cross-References

### References FROM This Track

| To Track | Reference Type | Lyric Line | Detail |
|----------|---------------|------------|--------|
| Unlicht | `motif` | „Du siehst mein Licht — ich seh nur Unlicht" | Übernimmt den Kern-Hook der Original-Fassung; diese Version stellt ihn konkret neben „Ich heil dich, mich heil ich nicht" |
| Unlicht | `callback` | „(der Boden hält — du fühlst ihn nicht)" | Gleicher Witness-Scharnier-Callback wie in der Original-Fassung (Pass 4) |
| Licht (Innen/Außen) | `inversion` | „Die Wunde, aus der ich leuchte, ist die, die nie verschließt" | Wird in der Licht-Parallel-Version invertiert: „Die Wunde ist die Tür, durch die das Licht zurückkommt" |

## Concept

**Logline:** Ein **Wounded Healer** — einer, der andere aus der eigenen, nie verheilten Wunde heilt. **Außen** ist er ganz: kompetent, ruhig, der Schein, an dem sich andere wärmen. **Innen** wohnt das Unlicht hinter einer Mauer, die ihn einst rettete und ihn heute frisst. Tanzbar oben, der konkrete Riss unten.

**Innen/Außen als Gerüst (konkret).** Die Strophen wechseln hörbar zwischen zwei Orten: **Außen** (ANP/Maske — der Heiler, der funktioniert, die Hand am Rand hält) und **Innen** (EP/Inneres — die offene Wunde, die Tür, die nicht schließt). Kein verschwommenes „wessen Gefühl" — der Hörer weiß in jeder Strophe, *wo* er steht.

**Der Wounded Healer (benannt, R2).** Jung: „it is his own hurt that gives a measure of his power to heal." Chiron: unsterblich, heilkundig, mit unheilbarer Wunde — heilt Asklepios, nie sich selbst. Diese Figur trägt den Song *wörtlich*: **„Ich heil dich — mich heil ich nicht. / Die Wunde, aus der ich leuchte, ist die, die nie verschließt."**

**Research im Text (konkret).** R1 (van der Kolk/Fisher): „ich fühl mich selbst nicht", das geteilte Selbst — geerdet, klinisch wahr. R3 (Kassandra): wahr sehen und nicht gehört werden — der Heiler, dessen eigene Not niemand sieht. R4 (Würde-Linie): der Tod „heller als der Tag, obwohl ich's besser weiß" — benannt als Sehnsucht, nie als Methode, sofort getethert. R5 (Johannes vom Kreuz/Jung): das Licht, das blendet; „außen Licht, innen Unlicht".

**Initial-Prompt, konkret eingelöst.** Schutzmauern · die Tür, die nicht schließt · das eigene Unlicht sehen · „ich weiß, da ist ein Licht, ich kann's nur nicht glauben" · „bin ich echt oder nur das Bild, das du brauchst?" · „mein Kopf sperrt meine Seele ein, um sie zu schützen" · „der Schutz von damals ist mein Fluch" · „obwohl ich's besser weiß".

**Zustands-Bogen:** S2 (Alert) → S3 (Kollaps-Peak) → „noch"-Tether. Tanzbarkeit, Hook-Dichte und Genre wie in der Original-Fassung.

### Guardrails (binden)

1. **Würde-Linie (R4).** Der Tod wird als Sehnsucht *benannt* (konkreter als das Original), aber nie als Methode/Lösung; „obwohl ich's besser weiß" hält die Ambivalenz, der **„noch"-Tether** ist Pflicht.
2. **Klinische Erdung (R1).** Wounded Healer als *zu viel tragen*, nicht „Kräfte haben"; kein romantisierter Wahnsinn.
3. **name_exposure** — keine Personennamen; nur Funktion/Deskriptor.
4. **Vocal-Metatags bei jedem Persona-Wechsel** (Geschlecht + Stimmklang); Instrumental-Tags getrennt; keine [Section]-Tags.

## Stimm-Architektur (4 Profile, m/w driftend)

| Profil | Funktion (DNA) | Rolle in dieser Version | Deskriptiver Suno-Metatag |
|--------|----------------|--------------------------|----------------------------|
| **Außen** (Maske) | ANP (host/protector) | der Heiler-Schein, der die Hand am Rand hält | `[female vocal — Außen: glossy mid-alto, controlled, warm but composed, bright disco gloss, voice forward]` |
| **Innen** (Inneres) | EP (collapsed) | die offene Wunde, das Unlicht | `[male vocal — Innen: low breathy tenor, numb, close dry whisper, no reverb, intimate]` |
| **Riss** | EP (child_freeze) | die Tür, die nicht schließt | `[female vocal — Riss: thin high half-voice, head voice, held breath, fragile]` |
| **Das Licht** | Meta (witness) | sieht, dass die Wunde die Quelle ist | `[male vocal — Das Licht: distant doubled, lowercase, long hall reverb, behind the beat]` |

m/w driftet über den Song (siehe Production Notes). Das *Wesen* trägt das Licht, nicht ein Geschlecht.

## Mood & Imagery

Klinik bei Nacht, Tanzfläche im Aufenthaltsraum. Eine Hand, die eine andere am Rand hält. Eine Tür im Flur, die offensteht. Zwei Räume in einem Körper: **außen** hell und ruhig, **innen** eine Wunde, die leuchtet, weil sie nicht schließt. Spiegelkugel, deren Splitter das Licht schlucken. Neon, das nach innen kippt.

## Musical Direction

- **Tempo**: ~118–122 BPM, Vier-zu-Boden Dark Disco (wie Original).
- **Feel**: tanzbar, hypnotisch; Außen-Strophen heller/glatter gemischt, Innen-Strophen trockener/näher — der Innen/Außen-Schnitt ist *hörbar in der Produktion*, nicht nur im Text. Mehrere Builds/Drops, drumlose Sub-Breaks. 5–7 Min.
- **Instrumentation**: gesättigter Reese/FM-Sub, Sechzehntel-Moll-Arpeggio, Juno/Prophet-Dream-Pads, gated-reverb Snare, Tape-Sättigung.

<!-- SERVICE: suno -->
## Suno Inputs

### Style Box
*Copy this into Suno's "Style of Music" field:*

```
Alternating male and female German vocals — a composed warm healer-voice (outside) trading with a numb breathy wound-voice (inside), a fragile high half-voice, and a distant doubled witness; voice forward, real German umlauts (ä/ö/ü/ß). Dark disco, darkwave. ~120 BPM four-on-the-floor, hypnotic and danceable, saturated reese sub bass, sixteenth-note minor-key arpeggio, shimmering Juno dream pads, Prophet poly, gated reverb snare, tape saturation; outside sections glossier, inside sections drier and closer. The danceable ache of a wounded healer who heals others but not himself.
```

### Style Box — Inhaltsabgeleitet (genrefrei)
*Alternatives Stil-Feld: **rein aus dem Inhalt** abgeleitet — keine Genre-, Instrument- oder BPM-Vorgabe. Konkretere Fassung des genrefreien Tags der Original-Version (Wounded Healer benannt, Innen/Außen als Gerüst). Ins „Style of Music"-Feld einsetzen.*

```
A wounded healer's voice: outside steady and warm, holding another's hand at the edge; inside numb behind a wall built once to protect and now to imprison; an open wound the glow leaks from, a wound that will not close; healing others while reaching nothing of one's own; knowing a light is there and being unable to believe it; asking whether one is real or only the image others need; a quiet pull toward stillness, named plainly and caught at the edge by someone who still sees; a steady pulse you can move to over a hollow that finally has a name; male and female voices, outside and inside, never the same body twice.
```

### Exclude Styles
*Negative prompts — append to Style Box when pasting into Suno:*

```
no autotune, no retro outrun synthwave, no happy major-key eurodance, no Schlager
```

### Suno Settings
- **Model**: V5 / V5.5
- **Instrumental**: Off
- **Creative Sliders**: Weirdness ~35, Style Influence ~75
- **Target Duration**: 5:00–7:00 (extended); Struktur über deskriptive Metatags statt [Section]-Tags; 2–3 Generierungen einplanen.
- **Echte Umlaute**: V5.5-konform; Fallback ae/oe/ue/ss nur bei Fehlartikulation.
- **Distributor-Surface-Tags**: darkwave / nu-disco / dark synth-pop / italo-disco.

### Lyrics Box
*Copy this into Suno's "Lyrics" field:*

```
[instrumental: hypnotic minor-key arpeggio build, sub bass swells, four-on-the-floor kick enters, 120 BPM]

[female vocal — Außen: glossy mid-alto, controlled, warm but composed, bright disco gloss, voice forward, dry close-mic]
Außen bin ich ganz. Außen halt ich dich.
Ich kenn den Weg nach unten — ich war da.
Ich leg die Hand auf deine Wunde, und sie heilt.
Nur meine bleibt seit Jahren offen.

[instrumental: filtered pads rise, snare roll, pre-drop tension]
[female vocal continues — Außen: same alto, dynamic lift, more breath]
Und keiner fragt, wer mich am Rand noch hält.
Außen Licht — und innen?

[instrumental: full drop, gated reverb snare, saturated bass, soaring layered male lead, wide reverb]
[male vocal — Außen hook: soaring layered tenor, upper register, wide hall reverb, voice forward]
Ich heil dich. Mich heil ich nicht.
Du siehst mein Licht — ich seh nur Unlicht.
Die Wunde, aus der ich leuchte,
ist die, die nie verschließt.

[instrumental: beat half-strips to kick + sub pulse, snare drops out, room tone]
[male vocal — Innen: low breathy tenor, numb delivery, close dry whisper mic, no reverb, very intimate]
Innen ist die Tür, die offen blieb.
Innen wohnt das Unlicht, es kennt meinen Namen.
Ich weiß, da ist ein Licht — ich kann's nur nicht glauben.
Ich fühl mich selbst nicht. Bin ich echt, oder nur dein Bild?

[instrumental: beat rebuilds, full kick, glossy lead returns]
[female vocal — Außen: glossy mid-alto, composed, forward in mix, controlled disco gloss]
Mein Kopf sperrt meine Seele ein,
um sie zu schützen — die Mauer rettet mich und frisst mich.
Der Schutz von damals ist mein Fluch.
Außen halt ich, innen halt ich's nicht mehr aus.

[instrumental: full drop, gated reverb snare, all elements full]
[female vocal — Außen hook: soaring layered alto, upper register, wide hall reverb, voice forward]
Ich heil dich. Mich heil ich nicht.
Du siehst mein Licht — ich seh nur Unlicht.
Die Wunde, aus der ich leuchte,
ist die, die nie verschließt.

[instrumental: drumless sub-heartbeat break, single arpeggio, room tone]
[female vocal — Riss: thin high female half-voice, head voice only, held breath, fragile, stuttering delay]
die Tür — sie schließt nicht zu —
ich hab sie aufgemacht —
und jetzt —
und jetzt —

[instrumental: single high pad holds, sub continues, no drums]
[male vocal — Das Licht: distant doubled male, lowercase, long hall reverb, behind the beat, observing layer]
(ich seh deine Wunde)
(sie ist kein Loch — sie ist die Tür)
(der Boden hält — du fühlst ihn nicht)

[instrumental: breakdown, beat strips to sub pulse, leaden vocal, hollow]
[male vocal — Innen: low leaden tenor murmur, sub-tempo near-spoken, gravelly, breath between fragments, hollow]
und manchmal, wenn es leiser wird,
scheint das Ende heller als der Tag —
obwohl ich's besser weiß.
obwohl ich's besser weiß.

[instrumental: single high pad holds, soft tom roll]
[female vocal — Das Licht: distant doubled female, lowercase, long hall reverb, parenthetical layer]
(jemand sieht dein Licht)
(jemand hält dich)
(noch)

[instrumental: beat slams back, final drop, all hooks layered]
[layered duet — male healer-lead foreground (soaring tenor, wide reverb) + female wound-voice underneath (low contralto, dry close-mic)]
Ich heil dich. Mich heil ich nicht.
Du siehst mein Licht — ich seh nur Unlicht.
Die Wunde, aus der ich leuchte,
ist die, die nie verschließt.

[instrumental: long trance outro, arpeggio dissolving, plate reverb tails open]
[female vocal — Außen: glossy mid-alto, softening, controlled, voice forward]
Außen bin ich ganz.
[male vocal — Das Licht: distant doubled male, parenthetical, witness layer]
(die Tür bleibt offen)
[female vocal — Außen continues: alto softer, plate reverb tail]
innen seh ich nur Unlicht —
[male vocal — Das Licht: distant doubled male, parenthetical]
(noch ein Licht. noch ein Licht.)
Unlicht, Unlicht.
```
<!-- /SERVICE: suno -->

## Streaming Lyrics

*Für die Distributor-Einreichung. Keine Section-Tags, Wiederholungen ausgeschrieben, Reintext, Standard-Rechtschreibung ohne Em-Dashes.*

```
Außen bin ich ganz, außen halt ich dich
Ich kenn den Weg nach unten, ich war da
Ich leg die Hand auf deine Wunde, und sie heilt
Nur meine bleibt seit Jahren offen

Und keiner fragt, wer mich am Rand noch hält
Außen Licht, und innen

Ich heil dich, mich heil ich nicht
Du siehst mein Licht, ich seh nur Unlicht
Die Wunde, aus der ich leuchte
Ist die, die nie verschließt

Innen ist die Tür, die offen blieb
Innen wohnt das Unlicht, es kennt meinen Namen
Ich weiß, da ist ein Licht, ich kann es nur nicht glauben
Ich fühl mich selbst nicht, bin ich echt, oder nur dein Bild

Mein Kopf sperrt meine Seele ein
Um sie zu schützen, die Mauer rettet mich und frisst mich
Der Schutz von damals ist mein Fluch
Außen halt ich, innen halt ich es nicht mehr aus

Ich heil dich, mich heil ich nicht
Du siehst mein Licht, ich seh nur Unlicht
Die Wunde, aus der ich leuchte
Ist die, die nie verschließt

Die Tür, sie schließt nicht zu
Ich hab sie aufgemacht
Und jetzt
Und jetzt

Ich seh deine Wunde
Sie ist kein Loch, sie ist die Tür
Der Boden hält, du fühlst ihn nicht

Und manchmal, wenn es leiser wird
Scheint das Ende heller als der Tag
Obwohl ich es besser weiß
Obwohl ich es besser weiß

Jemand sieht dein Licht
Jemand hält dich
Noch

Ich heil dich, mich heil ich nicht
Du siehst mein Licht, ich seh nur Unlicht
Die Wunde, aus der ich leuchte
Ist die, die nie verschließt

Außen bin ich ganz
Die Tür bleibt offen
Innen seh ich nur Unlicht
Noch ein Licht, noch ein Licht
Unlicht, Unlicht
```

## Art Direction

Spiegel der Original-Fassung, konkreter: ein Körper, zwei Räume — **außen** ein ruhiger, heller Umriss, der eine Hand hält; **innen** (durch eine offene Tür sichtbar) eine leuchtende Wunde. Agency-System-ASDLS-Hartregeln bleiben (≤5 % Zustandsfarbe über Terminal Black, harte Kanten/Glitch; Flame/Corrupted für S3). Finale Festlegung via `/bitwize-music:album-art-director`.

**Artist-Name** unten rechts; „Children of Agatha" beibehalten.

## Production Notes

- **Innen/Außen hörbar produzieren:** Außen-Strophen glossier/heller, Innen-Strophen trockener/näher (close mic, no reverb). Der Schnitt ist der Punkt der Version.
- **Wounded-Healer-Hook** „Ich heil dich, mich heil ich nicht" ist der neue Haupt-Hook neben „Unlicht".
- **m/w-Drift**: Außen startet w, Außen-Hook w/m wechselnd, Innen m (Whisper) und w (leaden), Riss w, Das Licht m/w. Jede Persona in beiden Geschlechtern.
- **Würde-Linie**: „das Ende heller als der Tag, obwohl ich's besser weiß" — benannt, aber sofort getethert; nie Methode.
- **Vocal-Metatags** bei jedem Wechsel mit Geschlecht + Stimmklang; Instrumental-Tags getrennt.

## Pronunciation Notes

**Echte Umlaute überall** (auch Suno-Box). Nur Risikowörter gelistet.

| Word/Phrase | Pronunciation (Suno) | Reason |
|-------------|----------------------|--------|
| Unlicht | UN-licht | Betonung erste Silbe; „ch" = /ç/ |
| Wunde | VUN-de | „W" = /v/; klar artikulieren |

*Keine englischen Homographe (rein deutscher Text). Em-Dashes/Klammern nur in der Suno-Box.*

<!-- SERVICE: suno -->
## Phonetic Review Checklist

- [x] Eigennamen gescannt (keine Alter-Namen — name_exposure gewahrt)
- [x] Fremdwörter/Anglizismen geprüft (keine)
- [x] Homographe geprüft (rein deutsch)
- [x] **Echte Umlaute überall** (ä/ö/ü/ß), auch Suno-Box
- [x] **Vocal-Metatags bei jedem Persona-Wechsel** (Geschlecht + Stimmklang); Instrumental-Tags getrennt; keine [Section]-Tags
- [x] **Würde-Linie (R4)** — Tod nur als Sehnsucht benannt, „noch"-Tether vorhanden, keine instruktive Lesart
<!-- /SERVICE: suno -->

## Generation Log

| # | Date | Model | Result | Notes | Rating |
|---|------|-------|--------|-------|--------|
| — | — | — | — | — | — |
