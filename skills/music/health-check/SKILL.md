...
1. Use the `ToolSearch` tool with query `select:mcp__plugin_agency-system_agency-system-mcp__music_health_check` to load the MCP tool schema
2. Call `mcp__plugin_agency-system_agency-system-mcp__music_health_check` (the MCP tool, not a CLI command)
