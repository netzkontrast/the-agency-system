...
summary: Working canvas v2 for the minimal isomorphic agency-system base — one central routing skill, one codemode MCP, five+1 domains, schemas everywhere, graph-discovery, hook chain, three-layer harness ladder. Absorbs independent reviewer critique of v1.
---

# Agency-System base — minimal isomorphic canvas (v2)

> **Working brainstorm, not a spec yet.** Decisions are marked `[D-NN]`. Each has options + recommendation. User redirects at any `[D-NN]`. v2 absorbs the independent reviewer's critique of v1 (graph-discovery restored, harness ladder restored, hook chain restored, four decisions flipped, hand-waving spots specified).
