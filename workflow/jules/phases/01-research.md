...
The research phase is the first step in the jules row's two-phase
workflow. Its job is to take a `topic` (a short subject keyword) and
produce one or more `Finding` nodes in the ontology graph that capture
what the query surfaced. The phase delegates to the row's
`mcp__jules_query` MCP tool — the v0.1 implementation lives at
`agentic/jules/tools/query.py` and returns a placeholder finding so the
cell-loader / hook / ontology wiring can be exercised end-to-end before
real retrieval lands in a later phase.
