...
from . import _shared
from ._shared import _safe_json, get_plugin_version as _read_plugin_version
...
def _find_plugin_cache_dir() -> Path | None:
    """Locate the Claude Code plugin cache directory for agency-system.

    Scans ``~/.claude/plugins/cache/agency-system/`` for versioned
    subdirectories and returns the one with the highest version number.
    Returns ``None`` if no cache directory exists.
    """
    cache_base = Path.home() / ".claude" / "plugins" / "cache" / "agency-system"
...
def _check_skill_registration() -> dict[str, Any]:
    ...
    ``~/.claude/plugins/cache/agency-system/*/skills/*/SKILL.md``.
    ...
    if cache_dir is None:
        return {
            "status": "no_cache",
            "message": "No Claude Code plugin cache found for agency-system",
            "source_count": len(source_skills),
            "fix_message": (
                "Install or update the plugin: claude plugin update agency-system "
    ...
    if status == "stale":
        result["fix_message"] = (
            "Plugin cache is stale — run: claude plugin update agency-system "
            "— or use --plugin-dir for local development"
        )
...
async def music_get_plugin_version() -> str:
...
async def music_check_venv_health() -> str:
    ...
    if not venv_python.exists():
        return _safe_json({
            "status": "no_venv",
            "message": "Venv not found at ~/.agency-system/venv",
        })
    ...
    if status == "stale":
        result["fix_command"] = (
            f"~/.agency-system/venv/bin/pip install -r {req_path}"
        )

    return _safe_json(result)


async def music_health_check() -> str:
    """Run startup health checks: venv packages and skill registration.

    Combines music_check_venv_health and skill registration checks into a
    single call for session startup. Use this instead of calling
    music_check_venv_health directly during session start.
    ...
    # --- Venv check ---
    venv_raw = json.loads(await music_check_venv_health())
    ...
    if venv_status == "ok":
        ...
        checks.append({"name": "venv", "status": "fail",
                        "detail": "Venv not found at ~/.agency-system/venv"})
    else:
        checks.append({"name": "venv", "status": "fail",
...
def _check_state_cache() -> dict[str, Any]:
    ...
    cache_path = Path.home() / ".agency-system" / "cache" / "state.json"
...
async def music_diagnose_issue() -> str:
    ...
    venv_result = json.loads(await music_check_venv_health())
    ...
    # Add version check
    version_result = json.loads(await music_get_plugin_version())
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_get_plugin_version)
    mcp.tool(tags={"domain:music"})(music_check_venv_health)
    mcp.tool(tags={"domain:music"})(music_health_check)
    mcp.tool(tags={"domain:music"})(music_diagnose_issue)
