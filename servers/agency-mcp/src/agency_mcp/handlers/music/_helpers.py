...
from . import _shared
from ._shared import _normalize_slug
from ._shared import _resolve_audio_dir as _resolve_audio_dir  # noqa: F401
...
def _build_title_map(album_slug: str, wav_files: list[Path]) -> dict[str, str]:
    ...
    """
    from agency_mcp.tools.shared.text_utils import sanitize_filename, slug_to_title
...
def _check_cloud_enabled() -> str | None:
    ...
    try:
        ...
        return (
            "Could not load config. Ensure ~/.agency-system/config.yaml exists."
        )
    if not config:
        return "Config not found. Run /agency-system:configure first."
    cloud_config = config.get("cloud", {})
    if not cloud_config.get("enabled", False):
        return (
            "Cloud uploads not enabled. "
            "Set cloud.enabled: true in ~/.agency-system/config.yaml. "
