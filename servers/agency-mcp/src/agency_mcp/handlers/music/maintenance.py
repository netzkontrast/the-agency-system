...
from . import _shared
from ._shared import _normalize_slug, _resolve_audio_dir, _safe_json
...
async def music_reset_mastering(
    album_slug: str,
...
async def music_cleanup_legacy_venvs(
    dry_run: bool = True,
) -> str:
    """Detect and remove stale per-tool virtual environments from ~/.agency-system/.

    Prior to 0.40.0, each tool had its own venv (mastering-env, promotion-env,
    cloud-env). These are now consolidated into a single ~/.agency-system/venv/.
    ...
    """
    tools_root = Path.home() / ".agency-system"
    results: dict[str, dict[str, Any]] = {}
    ...
    return _safe_json({
        ...
        "results": results,
        "note": "All tools now use ~/.agency-system/venv/ (unified venv).",
    })


async def music_migrate_audio_layout(
    album_slug: str = "",
    dry_run: bool = True,
) -> str:
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_reset_mastering)
    mcp.tool(tags={"domain:music"})(music_cleanup_legacy_venvs)
    mcp.tool(tags={"domain:music"})(music_migrate_audio_layout)
