...
async def music_transcribe_audio(
    album_slug: str,
    track_filename: str = "",
...
async def music_prepare_singles(
    album_slug: str,
    dry_run: bool = False,
    xml_only: bool = False,
) -> str:
    ...
    If source/ has a .manifest.json (from music_transcribe_audio), files are
    ...
    if not source_dir.is_dir():
        ...
        if sheet_dir.is_dir():
            ...
            return _safe_json({
                ...
                "suggestion": "Run music_transcribe_audio first to generate sheet music.",
            })

    singles_dir = audio_dir / "sheet-music" / "singles"

    prepare_mod = _helpers._import_sheet_music_module("music_prepare_singles")
    if prepare_mod is None:
        return _safe_json({"error": "Sheet music music_prepare_singles module not available."})
    _prepare_singles = prepare_mod.music_prepare_singles
    ...
    songbook_mod = _helpers._import_sheet_music_module("music_create_songbook")
    if songbook_mod is None:
        return _safe_json({"error": "Sheet music music_create_songbook module not available."})
    ...
    try:
        from agency_mcp.tools.shared.config import load_config
        cfg = load_config()
        if cfg:
    ...
    if cache_tracks:
        from agency_mcp.tools.shared.text_utils import sanitize_filename
        from agency_mcp.tools.shared.text_utils import slug_to_title as _s2t
        title_map = {}
...
async def music_create_songbook(
    album_slug: str,
    title: str,
    ...
    if singles_dir.is_dir():
        ...
        if sheet_dir.is_dir():
            ...
            return _safe_json({
                ...
                "suggestion": "Run music_transcribe_audio and music_prepare_singles first.",
            })

    songbook_mod = _helpers._import_sheet_music_module("music_create_songbook")
    if songbook_mod is None:
        return _safe_json({"error": "Sheet music music_create_songbook module not available."})
    _create_songbook = songbook_mod.music_create_songbook
    auto_detect_cover_art = songbook_mod.auto_detect_cover_art
...
async def music_publish_sheet_music(
    album_slug: str,
    ...
    if not sheet_music_dir.is_dir():
        return _safe_json({
            ...
            "suggestion": (
                "Run music_transcribe_audio first to generate source files, "
                "then music_prepare_singles to create distribution-ready PDFs."
            ),
        })
    ...
    if not files_to_upload:
        return _safe_json({
            ...
            "suggestion": "Run music_prepare_singles and/or music_create_songbook first.",
        })
    ...
    try:
        ...
        return _safe_json({
            ...
            "suggestion": "Configure cloud.r2 or cloud.s3 credentials in ~/.agency-system/config.yaml",
        })
    ...
    try:
        ...
        return _safe_json({
            ...
            "suggestion": "Set cloud.r2.bucket or cloud.s3.bucket in ~/.agency-system/config.yaml",
        })
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_transcribe_audio)
    mcp.tool(tags={"domain:music"})(music_prepare_singles)
    mcp.tool(tags={"domain:music"})(music_create_songbook)
    mcp.tool(tags={"domain:music"})(music_publish_sheet_music)
