...
async def music_generate_promo_videos(
    album_slug: str,
    style: str = "pulse",
    duration: int = 15,
    track_filename: str = "",
    ...
    if not artwork:
        return _safe_json({
            ...
            "suggestion": "Place album.png in the audio directory or use /agency-system:import-art.",
            "looked_for": artwork_patterns,
        })

    from agency_mcp.tools.promotion.generate_promo_video import (
        batch_process_album,
        generate_waveform_video,
    )
    from agency_mcp.tools.shared.fonts import find_font
...
async def music_generate_album_sampler(
    album_slug: str,
    ...
    from agency_mcp.tools.promotion.music_generate_album_sampler import (
        music_generate_album_sampler as _gen_sampler,
    )
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_generate_promo_videos)
    mcp.tool(tags={"domain:music"})(music_generate_album_sampler)
