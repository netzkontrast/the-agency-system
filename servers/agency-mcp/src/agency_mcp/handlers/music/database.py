...
from . import _shared
from ._shared import _find_album_or_error, _normalize_slug, _safe_json
...
def _get_db_connection() -> tuple[Any, str | None]:
    ...
    """
    from agency_mcp.tools.database.connection import get_connection, get_db_config

    db_config = get_db_config()
    if db_config is None:
        return None, _safe_json({
            "error": "Database not configured or not enabled. "
                     "Add a 'database:' section to ~/.agency-system/config.yaml"
        })
...
async def music_db_init(run_migrations: str = "true") -> str:
...
async def music_db_list_tweets(
    album_slug: str = "",
    posted: str = "",
    enabled: str = "",
    platform: str = "",
...
async def music_db_create_tweet(
    album_slug: str,
    tweet_text: str,
    track_number: int = 0,
        ...
        if not album_row:
            return _safe_json({
                "error": f"Album '{album_slug}' not found in database. "
                         "Use music_db_sync_album to sync it first.",
            })
...
async def music_db_update_tweet(
    tweet_id: int,
    tweet_text: str = "",
    posted: str = "",
    enabled: str = "",
...
async def music_db_delete_tweet(tweet_id: int) -> str:
...
async def music_db_search_tweets(
    query: str,
    album_slug: str = "",
    platform: str = "",
    limit: int = 50,
...
async def music_db_sync_album(album_slug: str) -> str:
...
async def music_db_get_tweet_stats(album_slug: str = "") -> str:
...
def register(mcp: Any) -> None:
    mcp.tool(tags={"domain:music"})(music_db_init)
    mcp.tool(tags={"domain:music"})(music_db_list_tweets)
    mcp.tool(tags={"domain:music"})(music_db_create_tweet)
    mcp.tool(tags={"domain:music"})(music_db_update_tweet)
    mcp.tool(tags={"domain:music"})(music_db_delete_tweet)
    mcp.tool(tags={"domain:music"})(music_db_search_tweets)
    mcp.tool(tags={"domain:music"})(music_db_sync_album)
    mcp.tool(tags={"domain:music"})(music_db_get_tweet_stats)
