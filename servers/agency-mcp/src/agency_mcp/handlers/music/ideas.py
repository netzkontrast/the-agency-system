...
from . import _shared
from ._shared import _safe_json
...
async def music_update_idea(title: str, field: str, value: str) -> str:
...
async def music_promote_idea(
    ...
    # "kleine welt"), but music_update_idea and _set_promoted_to_field match the
    ...
    status_result = json.loads(
        await music_update_idea(canonical_title, "status", "In Progress")
    )
    ...
    try:
        ...
        logger.warning("music_promote_idea: cache rebuild failed: %s", e)
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_create_idea)
    mcp.tool(tags={"domain:music"})(music_update_idea)
    mcp.tool(tags={"domain:music"})(music_promote_idea)
