...
from . import _shared
from ._shared import _STREAMING_PLATFORMS, _find_album_or_error, _safe_json
...
async def music_get_streaming_urls(album_slug: str) -> str:
...
async def music_update_streaming_url(album_slug: str, platform: str, url: str) -> str:
    ...
    from agency_mcp.tools.state.parsers import parse_album_readme
...
async def music_verify_streaming_urls(album_slug: str) -> str:
        ...
        for method in ("HEAD", "GET"):
            try:
                req = urllib.request.Request(
                    url, method=method,
                    headers={
                        "User-Agent": "agency-system-mcp/1.0 (link checker)",
                    },
                )
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_get_streaming_urls)
    mcp.tool(tags={"domain:music"})(music_update_streaming_url)
    mcp.tool(tags={"domain:music"})(music_verify_streaming_urls)
