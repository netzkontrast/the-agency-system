...
from agency_mcp.handlers.music._shared import _STREAMING_PLATFORMS, _find_album_or_error, _safe_json
...
async def update_streaming_url(album_slug: str, platform: str, url: str) -> str:
    ...
    db_synced = False
    try:
        from agency_mcp.handlers.music.database import _check_db_deps, _get_db_connection
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(name="music_get_streaming_urls", tags=["domain:music"])(get_streaming_urls)
    mcp.tool(name="music_update_streaming_url", tags=["domain:music"])(update_streaming_url)
    mcp.tool(name="music_verify_streaming_urls", tags=["domain:music"])(verify_streaming_urls)
