"""Core query tools — albums, tracks, sessions, config, music_search, paths."""
...
async def music_find_album(name: str) -> str:
    ...
    if not albums:
        logger.info("music_find_album: no albums in cache, attempting auto-rebuild")
...
async def music_get_session() -> str:
...
async def music_update_session(
    album: str = "",
    track: str = "",
    phase: str = "",
    action: str = "",
    ...
    """
    session = _shared.cache.music_update_session(
        album=album or None,
        track=track or None,
        phase=phase or None,
...
async def music_rebuild_state() -> str:
...
async def music_get_config() -> str:
    ...
    if not config:
        return _safe_json({"error": "No config in state. Run music_rebuild_state first."})

    return _safe_json({"config": config})


async def music_get_python_command() -> str:
    ...
    Returns:
        JSON with:
            python: Absolute path to ~/.agency-system/venv/bin/python3
    ...
    """
    venv_python = Path.home() / ".agency-system" / "venv" / "bin" / "python3"
    ...
    if not venv_exists:
        result["warning"] = (
            "Venv not found at ~/.agency-system/venv. "
            "Create it with: python3 -m venv ~/.agency-system/venv && "
            "~/.agency-system/venv/bin/pip install pyloudnorm scipy numpy "
...
async def music_search_items(query: str, scope: str = "all") -> str:
    """Full-text music_search across albums, tracks, ideas, and skills.

    Args:
        query: Search query (case-insensitive substring match)
        scope: What to music_search - "albums", "tracks", "ideas", "skills", or "all" (default)
...
async def music_resolve_path(path_type: str, album_slug: str, genre: str = "") -> str:
...
async def music_resolve_track_file(album_slug: str, track_slug: str) -> str:
    """Find a track's file path and return its full metadata from state cache.

    More complete than music_get_track — includes the resolved file path and album context.
...
async def music_list_track_files(album_slug: str, status_filter: str = "") -> str:
    ...
    Unlike music_list_tracks, includes file paths and supports filtering by status.
...
async def music_extract_section(album_slug: str, track_slug: str, section: str) -> str:
...
async def music_update_track_field(
    album_slug: str,
    track_slug: str,
    field: str,
    value: str,
            ...
            if require_link and not track_data.get("has_suno_link", False):
                return _safe_json({
                    "error": "Cannot mark track as 'Final' — no Suno link set. "
                             "Set the Suno link first with music_update_track_field("
    ...
    if field_key in ("sources-verified", "sources_verified") and not force:
        ...
        if "verified" in val_lower and "pending" not in val_lower:
            ...
            if album_path:
                ...
                if sources_path.exists():
                    try:
                        ...
                        if _MARKDOWN_LINK_RE.music_search(sources_text):
                            has_links = True
            ...
            if not has_links:
                ...
                if source_section and _MARKDOWN_LINK_RE.music_search(source_section):
                    has_links = True
            if not has_links:
                return _safe_json({
                    "error": (
    ...
    match = pattern.music_search(text)
    if not match:
        return _safe_json({
...
async def music_get_album_progress(album_slug: str) -> str:
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_list_albums)
    mcp.tool(tags={"domain:music"})(music_get_track)
    mcp.tool(tags={"domain:music"})(music_list_tracks)
    mcp.tool(tags={"domain:music"})(music_get_session)
    mcp.tool(tags={"domain:music"})(music_update_session)
    mcp.tool(tags={"domain:music"})(music_rebuild_state)
    mcp.tool(tags={"domain:music"})(music_get_config)
    mcp.tool(tags={"domain:music"})(music_get_python_command)
    mcp.tool(tags={"domain:music"})(music_get_ideas)
    mcp.tool(tags={"domain:music"})(music_search_items)
    mcp.tool(tags={"domain:music"})(music_get_pending_verifications)
    mcp.tool(tags={"domain:music"})(music_resolve_path)
    mcp.tool(tags={"domain:music"})(music_resolve_track_file)
    mcp.tool(tags={"domain:music"})(music_list_track_files)
    mcp.tool(tags={"domain:music"})(music_extract_section)
    mcp.tool(tags={"domain:music"})(music_update_track_field)
    mcp.tool(tags={"domain:music"})(music_get_album_progress)
