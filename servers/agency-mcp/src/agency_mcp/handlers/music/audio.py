...
# running long music_master_album calls (up to 30+ min with ADM enabled) can
...
from . import _album_stages
from agency_mcp.tools.mastering.ceiling_guard import (
...
async def music_analyze_audio(album_slug: str, subfolder: str = "") -> str:
    ...
    from agency_mcp.tools.mastering.analyze_tracks import analyze_track
...
async def music_qc_audio(
    ...
    from agency_mcp.tools.mastering.qc_tracks import ALL_CHECKS, _resolve_click_thresholds, qc_track
...
async def music_master_audio(
    ...
    from agency_mcp.tools.mastering.config import build_effective_preset
    from agency_mcp.tools.mastering.master_tracks import (
...
async def music_fix_dynamic_track(album_slug: str, track_filename: str) -> str:
    ...
    from agency_mcp.tools.mastering.music_fix_dynamic_track import fix_dynamic
...
async def music_master_with_reference(
    album_slug: str,
    ...
    try:
        from agency_mcp.tools.mastering.reference_master import (
            music_master_with_reference as _ref_master,
        )
...
def _adm_adaptive_ceiling_per_track(
    ...
    Same math as the inline closure in music_master_album, but with explicit
    ...
    **Progress visibility:** stage-level progress is written to
    ...
    def _inject_notices_and_return(result: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "music_master_album: halt result is not valid JSON, "
    ...
    def _progress_log_stage(
        ...
        *, extra: str | None = None,
        ...
        try:
            ...
            logger.warning("music_master_album: progress log write failed: %s", exc)
...
async def music_render_codec_preview(
    album_slug: str,
    ...
    if not source_dir.is_dir():
        return _safe_json({
            "error": f"Source subfolder not found: {source_dir}",
            "hint": "Run music_master_audio or music_master_album first to populate mastered/.",
        })
    ...
    from agency_mcp.tools.mastering.codec_preview import CodecPreviewError, render_aac_preview
...
async def music_mono_fold_check(
    album_slug: str,
    subfolder: str = "mastered",
    ...
    import soundfile as sf
    from agency_mcp.tools.mastering.mono_fold import mono_fold_metrics
    from agency_mcp.tools.mastering.mono_fold_report import render_mono_fold_markdown
...
async def music_prune_archival(album_slug: str, keep: int = 3) -> str:
    ...
    written by music_master_album when mastering.archival_enabled is true.
    ...
    for f in to_remove:
        try:
            ...
            logger.warning("music_prune_archival: could not remove %s: %s", f, exc)
...
async def music_measure_album_signature(
    album_slug: str,
    subfolder: str = "mastered",
    genre: str = "",
    ...
    the music_album_coherence_check / music_album_coherence_correct tools in phase 3b.
    ...
    from agency_mcp.tools.mastering.analyze_tracks import analyze_track
    from agency_mcp.tools.mastering.album_signature import (
        build_signature,
    ...
    if anchor_requested:
        from agency_mcp.tools.mastering.anchor_selector import select_anchor
...
async def music_album_coherence_check(
    album_slug: str,
    subfolder: str = "mastered",
    genre: str = "",
    anchor_track: int | None = None,
    ...
    Runs the same measurement pipeline as music_measure_album_signature, then
    ...
    Read-only — no files modified. Use music_album_coherence_correct to
    ...
    if not genre and anchor_track is None:
        return _safe_json({
            "error": (
                "music_album_coherence_check requires either a genre (for "
                "tolerances + anchor selection) or an explicit anchor_track "
                "(falls back to default tolerances with a warning)."
            ),
        })

    from agency_mcp.tools.mastering.coherence import (
        classify_outliers,
        load_tolerances,
    )
...
async def music_album_coherence_correct(
    album_slug: str,
    genre: str,
    ...
    First runs the same logic as music_album_coherence_check to identify
    ...
    Args:
        ...
        target_lufs / ceiling_db / cut_highmid / cut_highs: Mastering
            overrides — same semantics as music_master_album. Used only as
    ...
    if not genre:
        return _safe_json({
            "error": "music_album_coherence_correct requires a genre for tolerance + preset resolution.",
        })
    ...
    if not polished_dir.is_dir():
        return _safe_json({
            "error": (
                ...
                f"Run polish_audio first, then music_master_album, then retry."
            ),
        })
    ...
    pre_json = await music_album_coherence_check(
        album_slug=album_slug,
    ...
    from agency_mcp.tools.mastering.coherence import build_correction_plan, load_tolerances
    from agency_mcp.tools.mastering.config import build_effective_preset
    ...
    # (parity with music_master_album's _stage_coherence_correct — without this,
    ...
    from agency_mcp.tools.mastering.config import build_effective_preset
    from agency_mcp.tools.mastering.master_tracks import master_track
    ...
    post_json = await music_album_coherence_check(
        album_slug=album_slug,
        subfolder=check_subfolder,
        genre=genre,
        anchor_track=anchor_track,
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_analyze_audio)
    mcp.tool(tags={"domain:music"})(music_qc_audio)
    mcp.tool(tags={"domain:music"})(music_master_audio)
    mcp.tool(tags={"domain:music"})(music_fix_dynamic_track)
    mcp.tool(tags={"domain:music"})(music_master_with_reference)
    mcp.tool(tags={"domain:music"})(music_master_album)
    mcp.tool(tags={"domain:music"})(music_render_codec_preview)
    mcp.tool(tags={"domain:music"})(music_mono_fold_check)
    mcp.tool(tags={"domain:music"})(music_prune_archival)
    mcp.tool(tags={"domain:music"})(music_measure_album_signature)
    mcp.tool(tags={"domain:music"})(music_album_coherence_check)
    mcp.tool(tags={"domain:music"})(music_album_coherence_correct)
