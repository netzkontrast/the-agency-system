...
from ._shared import _find_wav_source_dir, _is_path_confined, _safe_json
from . import _helpers

logger = logging.getLogger(__name__)


async def music_polish_audio(
    album_slug: str,
    genre: str = "",
    use_stems: bool = True,
    dry_run: bool = False,
    ...
    Args:
        ...
        analyzer_results: Optional pre-computed per-track/per-stem
            analyzer output from `music_analyze_mix_issues`. When None and
            ...
            music_polish_album passes its existing analyze-stage output
    ...
    from agency_mcp.tools.mixing.mix_tracks import (
        discover_stems,
        load_mix_presets,
        mix_track_full,
        mix_track_stems,
    ...
    # music_polish_audio calls still see the coupling). music_polish_album skips
    # this by passing its existing analyze-stage output down.
    if analyzer_results is None and not dry_run:
        analyzer_json = await music_analyze_mix_issues(album_slug, genre)
        ...
        if "error" in analyzer_parsed:
            ...
            logger.warning(
                "music_polish_audio analyzer auto-run failed for album %r (genre=%r): %s",
...
def _resolve_analyzer_peak_ratio(
    ...
    `music_analyze_mix_issues` output and polish output (#323 follow-up).
...
def _resolve_analyzer_thresholds() -> tuple[float, float, bool]:
    ...
    """
    try:
        from agency_mcp.tools.mixing.mix_tracks import load_mix_presets
    except ImportError:
...
def _build_analyzer(
    ...
    `music_analyze_mix_issues` lets tests exercise the logic without mounting
    an album directory.
        ...
        if total_energy > 0:
            ...
            if high_mid_ratio > harsh_ratio:
                ...
                if adm_aware_excitation:
                    ...
                    try:
                        from agency_mcp.tools.mixing.mix_tracks import MIX_PRESETS
                    except ImportError:
                        MIX_PRESETS = {}
...
async def music_analyze_mix_issues(
    album_slug: str,
    genre: str = "",
) -> str:
    ...
    if not wav_files:
        stems_dir = audio_dir / "stems"
        if stems_dir.is_dir():
            from agency_mcp.tools.mixing.mix_tracks import discover_stems
            track_dirs = sorted([d for d in stems_dir.iterdir() if d.is_dir()])
...
async def music_polish_album(
    ...
    # --- Stage 1: Analysis ---
    analysis_json = await music_analyze_mix_issues(album_slug)
    ...
    # run — music_polish_audio would otherwise re-invoke music_analyze_mix_issues).
    polish_json = await music_polish_audio(
        album_slug=album_slug,
        genre=genre,
        use_stems=use_stems,
        dry_run=False,
    ...
    from agency_mcp.tools.mastering.qc_tracks import qc_track
...
async def music_polish_and_master_album(
    album_slug: str,
    genre: str = "",
    ...
    Runs music_polish_album() to clean up Suno audio, then master_album() with
    ...
    """
    from ..audio import master_album

    polish_json = await music_polish_album(album_slug=album_slug, genre=genre)
...
def register(mcp: Any) -> None:
    ...
    mcp.tool(tags={"domain:music"})(music_polish_audio)
    mcp.tool(tags={"domain:music"})(music_analyze_mix_issues)
    mcp.tool(tags={"domain:music"})(music_polish_album)
    mcp.tool(tags={"domain:music"})(music_polish_and_master_album)
