# Jules Plugin Smoke Test Transcript
...
**Command:**
```bash
claude --plugin-dir ./jules-plugin --help
```
...
**Command:**
```bash
JULES_API_KEY=mock PYTHONPATH=jules-plugin/mcp-server/src python3 -m jules_mcp.server &
server_pid=$!
sleep 2
kill $server_pid
```
...
│                            https://gofastmcp.com                             │
│                                                                              │
│                  🖥  Server:      jules-orchestrator, 3.3.1                   │
│                  🚀 Deploy free: https://horizon.prefect.io                  │
│                                                                              │
╰──────────────────────────────────────────────────────────────────────────────╯


[INFO] Starting MCP server 'jules-orchestrator' with transport 'stdio'
```
(No tracebacks)
...
**Verdict:** FAIL (Known Issue)

**Output:**
```
['execute', 'get_schema', 'search']
```
*Note: Due to FastMCP `CodeMode` transform, the server's originally registered `jules_*` tools are hidden and replaced by `search`, `get_schema`, and `execute`.*
...
Traceback (most recent call last):
  ...
ValueError: Tool jules_list not found
```
*Note: The tool `jules_list` is not found via the transformed standard provider interface because of the `CodeMode` replacement.*
