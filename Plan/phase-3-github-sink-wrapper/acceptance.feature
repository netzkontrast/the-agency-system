Feature: Phase 3 — GitHub MCP summary wrappers
  ...
  Background:
    ...
    And the codemode registry has indexed all tools tagged with "subagent:true"
  ...
  Scenario: The main session never sees the raw PR body
    ...
    When distillation happens in an ephemeral subagent with a scoped tool allow-list
    Then the main session does not receive the raw mcp__github__pull_request_read payload
    And the main session receives only the distilled summary payload
  ...
  Scenario Outline: The subagent tool allow-list excludes every write/destructive GitHub tool
    ...
    When the ephemeral subagent is dispatched
    ...
    And the subagent tool allow-list must NOT contain "mcp__github__pull_request_review_write"
    And the subagent tool allow-list must NOT contain "mcp__github__issue_write"
    And the subagent tool allow-list must NOT contain "mcp__github__pull_request_write"
    And the subagent tool allow-list must NOT contain "mcp__github__push"
