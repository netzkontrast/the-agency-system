...
Once all four domains are registered, the unified plugin will expose ~180 tools (~67 music + ~73 novel + ~12 jules + ~32 agentic). (Note: "four domains" here refers to the four domains with anchor-triad eager registrations at the time this spec was authored — `music`, `novel`, `jules`, `agentic`. Per `Plan/harness/VOCABULARY.md` §4.1, the canonical handler-bearing domain count is now five — `music`, `novel`, `jules`, `context`, `shared` — plus the `agentic` skill-only domain; `context` and `shared` were added after this spec was written.) Without intervention, every `tools/list` MCP call dumps every tool schema into the model's context — measured at ~34k tokens in the bitwize baseline. Code Mode + `defer_schema=True` is the FastMCP feature that solves this: ~4 anchor tools per domain register eagerly (their schemas always loaded), the rest register deferred (schema fetched on demand via `tool_search` / `get_schema` / `execute` meta-tools). The bitwize baseline target is ~315 tokens. This spec ships the classification manifest, the registry helper that decorates tools per the manifest, the graceful-fallback CodeMode import (mirroring the jules-plugin pattern), and the integration test that asserts the boot token budget. It is the single most token-impactful spec in Wave A — without it, the plugin is unusable at scale.
