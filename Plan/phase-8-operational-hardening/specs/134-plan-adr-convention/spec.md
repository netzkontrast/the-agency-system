...
1. **Gate 1 — Confidence.** Verify `Plan/decisions/` and `Plan/_templates/adr-template.md` do not exist (`ls Plan/decisions Plan/_templates/adr-template.md 2>&1 | grep -i "no such file"`). Confirm `Plan/_lint/check_affects.py` exists (spec 099 prerequisite). Confirm `tests/unit/lint/` exists (created by spec 099). Read the agency repo's `decisions/readme.md` and `tools/adr/schema.py` once for the MADR shape; cite both reads in the Confidence table.
