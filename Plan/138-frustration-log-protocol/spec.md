...
- [ ] `Plan/JULES_PROTOCOL.md` Appendix gains a **"Friction Root-Cause Taxonomy" subsection** with the 8 categories agency catalogued (`Plan/_research/agency-repo-analysis/findings.md` §10): spec-without-validator, reciprocity drift, two-source-of-truth drift, parallel-dispatch worktree inconsistency, pre-existing baseline errors, prompt-ambiguity, schema-contract drift, structural-bloat tension. The rubric author may cite one of these tags when justifying the FL severity.
- [ ] **Structural-bloat is automatically FL2** (`Plan/_research/agency-repo-analysis/findings.md` §1.5 / agency `FRUSTRATED.md:32`): deeply nested folders with < 3 files per level, or per-file readme-only commits, MUST be logged at FL2 minimum. The lint script SHOULD detect this pattern in the affected files and emit a warning (`FL.2:structural-bloat-suspected:<path>`) but does not gate on it.
- [ ] **WARN→STRICT mode toggle**: the lint script's behaviour on a missing-but-not-malformed friction log is advisory (`FL.1:warning:missing-heading`, exit 0) by default; setting `AGENCY_FL_STRICT=1` flips it to error (exit non-zero). Rationale: agency learned to ship advisory-first, gate-after-corpus-clean (`/tmp/agency-research/FRUSTRATED.md:40`). Document the toggle in `Plan/JULES_PROTOCOL.md` Appendix; CI promotes to strict only after a clean rolling 7-day window of FL declarations.
